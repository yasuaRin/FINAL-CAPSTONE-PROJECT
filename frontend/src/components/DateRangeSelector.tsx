...
export const DateRangeSelector: React.FC<DateRangeSelectorProps> = ({ value, onChange }) => {
  ...
  const handleCalendarSelect = (range: RDPDateRange | undefined) => {
    if (range?.from && range?.to) {
      ...
      // We don't close immediately to let user see selection
    } else if (range?.from) {
      onChange({
        ...value,
        start: startOfDay(range.from),
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 10 }}
            className={`absolute right-0 mt-2 bg-background border border-border rounded-2xl shadow-xl z-50 overflow-hidden ${showCalendar ? 'w-[320px]' : 'w-56'}`}
          >
            {!showCalendar ? (
              <div className="p-2">
                {presets.map((preset) => (
                  ...
                  >
                    ...
                    {value.preset === preset.value && <Check size={14} className="text-primary" />}
                  </button>
                ))}
              </div>
            ) : (
