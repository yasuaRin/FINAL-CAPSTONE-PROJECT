    ...
    const handleCallback = async () => {
      try {
        ...
        const tokenPart = fullHash.includes('access_token=')
          ? fullHash.substring(fullHash.indexOf('access_token=') - 1)
          : null;
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/team_members?auth_user_id=eq.${sessionData.user.id}&select=role,status&limit=1`,
          {
            headers: {
