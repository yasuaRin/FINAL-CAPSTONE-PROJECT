...
const RoleBadge = ({ role }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color }}>{role?.replace("_", " ") || "staff"}</span>
    </div>
  );
};
...
export default function Team() {
  ...
  const handleSubmit = async (e) => {
    ...
    const name = fd.get("name"),
      email = fd.get("email") || editingMember?.email;
    const phone = fd.get("phone"),
      role = fd.get("role") || editingMember?.role || "staff";
      ...
      >
        ...
        >
          <div style={{ position: "relative", width: "100%", maxWidth: 340 }}>
            ...
            <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search members..." style={{ ...inputStyle, paddingLeft: 36, paddingRight: 16 }} />
          </div>
          <div
            style={{
              display: "flex",
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, textAlign: "center" }}>
              ...
              <div style={{ display: "flex", gap: 12, width: "100%" }}>
                <button onClick={() => setDeleteTarget(null)} style={cancelBtn}>
                  Cancel
                </button>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, marginBottom: 24 }}>
                ...
                {formAvatar && !avatarUploading && (
                  <button
                    ...
                    onClick={async () => {
                      try {
                        await deleteFromStorage(formAvatar);
                      } catch (_) {}
                      setFormAvatar(null);
                      formAvatarRef.current = null;
                      if (fileInputRef.current) fileInputRef.current.value = "";
                ...
                <div style={{ display: "flex", gap: 12 }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 13,
                      cursor: "pointer",
                      opacity: submitting || avatarUploading ? 0.7 : 1,
                    }}
                  >
